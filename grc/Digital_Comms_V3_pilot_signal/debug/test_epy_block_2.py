...
class blk(gr.sync_block):
    ...
    def count_available_bytes(self):
        ...
        if n_bytes == 0:
            print(
                f'[BER] count_available_bytes waiting_for_8_bits buffer_bits={len(self.count_buffer)}',
                flush=True,
            )
            return
        ...
        print(
            f'[BER] count_available_bytes buffer_bits={len(usable)} packed_bytes={len(packed)} '
            f'byte_errors={byte_errors} bit_errors={bit_errors}',
            flush=True,
        )

        if self.total_bits == 0:
            print('[BER] counters have not advanced yet.', flush=True)
    ...
    def work(self, input_items, output_items):
        ...
        if n > 0:
            preview = bits_in[:min(16, n)].tolist()
            print(f'[BER] work input_preview={preview}', flush=True)
        ...
        if not self.synced and len(self.bit_buffer) >= self.buffer_size:
            ...
            if not self.synced:
                print(
                    f'[BER] align_not_ready buffer_len={len(self.bit_buffer)} '
                    f'threshold={self.buffer_size}',
                    flush=True,
                )
        ...
        print(
            f'[BER] work exit call={self.work_calls} last_ber={self.last_ber:.6f} '
            f'last_byter={self.last_byter:.6f} bit_buffer={len(self.bit_buffer)} '
            f'count_buffer={len(self.count_buffer)}',
            flush=True,
        )
        return n
